...
const PhotoUploader = ({addedPhotos, onChange}) => {
    ...
    async function addPhotoUsingLink(e) {
      ...
      function removePhoto(e,filename){ //Silvia
        e.preventDefault();
        onChange([...addedPhotos.filter(photo => photo != filename)]);
      }


      function selectAsMainPhoto(e,filename){ //Silvia
        e.preventDefault();
        ...
        <div className="flex gp-2">
            ...
            <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {addedPhotos.length > 0 && addedPhotos.map(link => (
                ...
                <button onClick={e => removePhoto(e,link)} className="cursor-pointer absolute bottom-2 right-1 text-white bg-black bg-opacity-50rounded-2xl py-2 px-3">
                ...
                <button onClick={e => selectAsMainPhoto(e,link)} className="cursor-pointer absolute bottom-1 left-1 text-white bg-black bg-opacity-50rounded-2xl py-2 px-3">
                {link === addedPhotos[0] && (
